...
public abstract partial class WebTorrentTestBase
{
    ...
    public async Task Bep20_PeerIdFormat()
    {
        ...
        if (prefix != expectedPrefix) throw new Exception($"Prefix: '{prefix}', expected '{expectedPrefix}'");
    ...
    public async Task Bep20_PeerId_Format()
    {
        ...
        // Azureus-style: -SDMMNN- prefix derived from assembly version
        var prefix = System.Text.Encoding.ASCII.GetString(peerId, 0, 8);
        if (!prefix.StartsWith("-SD")) throw new Exception($"PeerId should start with -SD, got {prefix}");
        var v = typeof(WebTorrentClient).Assembly.GetName().Version ?? new Version(0, 0, 0);
        var expectedPrefix = $"-SD{v.Major * 10 + v.Minor:D2}{v.Build:D2}-";
        if (prefix != expectedPrefix) throw new Exception($"PeerId prefix: '{prefix}', expected '{expectedPrefix}'");
    ...
    // ═══════════════════════════════════════════════════════════
    //  BEP 15 — UDP Tracker Binary Format
    // ═══════════════════════════════════════════════════════════

    [TestMethod]
    public async Task Bep15_BigEndian_Int32()
    {
        // Verify big-endian int32 encoding (same logic as UdpTrackerClient)
        var buf = new byte[4];
        ...
        if (buf[0] != 0x01 || buf[1] != 0x02 || buf[2] != 0x03 || buf[3] != 0x04)
            throw new Exception("Int32 big-endian encoding wrong");

        // Read back
        int read = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
        if (read != value) throw new Exception($"Int32 round-trip: expected {value}, got {read}");

        Console.WriteLine("[BEP15] Big-endian int32: OK");
    }

    [TestMethod]
    public async Task Bep15_BigEndian_Int64()
    {
        // Verify big-endian int64 encoding (protocol_id = 0x41727101980)
        long protocolId = 0x41727101980L;
        var buf = new byte[8];
        buf[0] = (byte)(protocolId >> 56);
        buf[1] = (byte)(protocolId >> 48);
        buf[2] = (byte)(protocolId >> 40);
        buf[3] = (byte)(protocolId >> 32);
        buf[4] = (byte)(protocolId >> 24);
        buf[5] = (byte)(protocolId >> 16);
        buf[6] = (byte)(protocolId >> 8);
        buf[7] = (byte)protocolId;

        // Read back
        long read = ((long)buf[0] << 56) | ((long)buf[1] << 48) | ((long)buf[2] << 40) |
                    ((long)buf[3] << 32) | ((long)buf[4] << 24) | ((long)buf[5] << 16) |
                    ((long)buf[6] << 8) | buf[7];
        if (read != protocolId) throw new Exception($"Int64 round-trip: expected {protocolId}, got {read}");

        Console.WriteLine("[BEP15] Big-endian int64: OK");
    }

    [TestMethod]
    public async Task Bep15_ConnectRequest_Format()
    {
        // BEP 15: Connect request is exactly 16 bytes
        ...
        // Write protocol_id
        request[0] = (byte)(protocolId >> 56);
        request[1] = (byte)(protocolId >> 48);
        request[2] = (byte)(protocolId >> 40);
        request[3] = (byte)(protocolId >> 32);
        request[4] = (byte)(protocolId >> 24);
        request[5] = (byte)(protocolId >> 16);
        request[6] = (byte)(protocolId >> 8);
        request[7] = (byte)protocolId;
        // Write action
        request[8] = (byte)(actionConnect >> 24);
        request[9] = (byte)(actionConnect >> 16);
        request[10] = (byte)(actionConnect >> 8);
        request[11] = (byte)actionConnect;
        // Write transaction_id
        request[12] = (byte)(txId >> 24);
        request[13] = (byte)(txId >> 16);
        request[14] = (byte)(txId >> 8);
        request[15] = (byte)txId;

        if (request.Length != 16) throw new Exception("Connect request should be 16 bytes");
        // Verify action = 0
        if (request[8] != 0 || request[9] != 0 || request[10] != 0 || request[11] != 0)
            throw new Exception("Action should be 0 for connect");

        Console.WriteLine("[BEP15] Connect request format: OK");
    }

    [TestMethod]
    public async Task Bep15_AnnounceRequest_Format()
    {
        // BEP 15: Announce request is exactly 98 bytes
        var request = new byte[98];

        // Verify all fields fit
        if (request.Length != 98) throw new Exception("Announce request should be 98 bytes");

        // Field layout verification (offsets per BEP 15 spec):
        // 0-7: connection_id (8 bytes)
        // 8-11: action=1 (4 bytes)
        // 12-15: transaction_id (4 bytes)
        // 16-35: info_hash (20 bytes)
        // 36-55: peer_id (20 bytes)
        // 56-63: downloaded (8 bytes)
        // 64-71: left (8 bytes)
        // 72-79: uploaded (8 bytes)
        // 80-83: event (4 bytes)
        // 84-87: IP address (4 bytes)
        // 88-91: key (4 bytes)
        // 92-95: num_want (4 bytes)
        // 96-97: port (2 bytes)
        // Total: 98 bytes ✓

        Console.WriteLine("[BEP15] Announce request format (98 bytes): OK");
    }
