...
- **Pure-v2-only multi-file piece addressing.** `TorrentParser` populates `PieceHashes` from `FileRoots[0]`'s piece layer only for pure-v2-only torrents (hybrids use the v1 `pieces` field which covers all pieces correctly). A pure-v2 multi-file torrent round-tripped through the parser has `PieceHashes` covering only the first file. The deeper issue is that the current `Torrent.Download.cs` uses a global-piece-index model (`_hashes[index]`, piece 0 of the whole torrent) which maps cleanly onto hybrids via pad-file alignment but doesn't map onto pure v2's per-file piece layers without a separate per-file-piece-index download engine. **Impact:** hybrid torrents work everywhere (our default); pure-v2-only multi-file torrents can be parsed and their info hashes verified, but piece-by-piece download past file 0 would fail. Pure-v2 single-file works. Since `HuggingFaceProxy` and the default `TorrentCreator` configuration emit hybrids, no production consumer hits this today. Fix requires either (a) adding per-file piece indexing to `Torrent.Download.cs` + `Torrent.VerifyPieceHash` (global-index → file-index + file-piece-index), or (b) synthesizing pad files during parse to align with the global-piece-index model. Option (a) is the cleaner long-term path and is a Phase 3 item if we ever need pure-v2-only multi-file downloads.

- **Phase 2c step 2 leaf-level (`base_layer = 0`) hash_request serving.** Current `TryBuildV2HashesPayload` only serves `base_layer == pieceLayerLevel`. Leaf-level requests require re-hashing piece content from the chunk store. Straightforward extension; consumers rarely need it because piece-layer hashes are sufficient for full-piece verification. Consumers that want finer-grained (sub-piece) verification would need this path.
