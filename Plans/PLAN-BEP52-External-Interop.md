...
- [x] Step 4: End-to-end cross-seeding — hash + piece-verify cross-validated both ways 2026-04-24 (SpawnDev generates, libtorrent parses + verifies; corpus fixtures confirm libtorrent generates, SpawnDev parses). **Live-swarm bi-directional active seeding achieved 2026-04-24 evening** via `interop_test/qbittorrent_liveswarm.cs`: qBittorrent seeds + SpawnDev.WebTorrent C# client downloads over localhost TCP, SHA-256 byte-identical on the full 1 MiB hybrid torrent in under 3 seconds. Uncovered a latent `Torrent.AddPeer` bug where already-connected peers never fired the BT handshake (fix landed in the same commit). Reverse direction (seed-C# / leech-qBittorrent) is a symmetric follow-up - our client would need a TCP listener accepting qBittorrent's inbound connect.
