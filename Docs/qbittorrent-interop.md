...
**Does NOT prove:** peer-wire exchange over a live network. That's the live-swarm step below — also PASSING.
...
| `js_webtorrent_liveswarm.cs` | JS WebTorrent live-swarm interop — PASSING. Spawns `SpawnDev.RTC.ServerApp.exe` as a tracker subprocess, generates a fresh hybrid torrent pointing at it, launches Node.js `webtorrent@^2` + `@roamhq/wrtc` as the seeder, runs SpawnDev.WebTorrent C# as the leecher. 1 MiB SHA-256 byte-identical via real WebRTC datachannel. See "JS WebTorrent interop" section below for full details. |
| `qbittorrent_reverse_liveswarm.cs` | Reverse-direction live-swarm — PASSING. SpawnDev.WebTorrent C# seeds via `TcpListenerService`; qBittorrent leeches via `/api/v2/torrents/addPeers` pointing at our listener. Closes the seed-C# / leech-mainline interop direction. 1 MiB SHA-256 byte-identical. |
