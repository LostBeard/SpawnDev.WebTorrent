...
- [x] `HuggingFaceProxy` generator switches to hybrid output. SHIPPED in `308bb65`. `HuggingFaceProxy.cs` line 263-264 passes `MetaVersion = 2 + Hybrid = true` to `TorrentCreator.CreateFromFileAsync`, routing every HF model-file torrent through the streaming hybrid v1+v2 path. Checkbox was missed - the Phase 2b status line below already says SHIPPED.
...
- [x] Interop tests: SHIPPED 2026-04-23 (`a73b68c`, `07d27c3`, `8736c44`). 4 libtorrent 2.0 corpus fixtures embedded in `Demo.Shared/InteropFixtures/` (v1.torrent, v2.torrent, hybrid.torrent, v2_multipiece_file.torrent, v2_only.torrent, plus `manifest.json` with expected values). Tests in `WebTorrentTestBase.LibtorrentInteropTests.cs` prove SpawnDev's v2 info hash byte-matches libtorrent's canonical output against the corpus. `regenerate_fixtures.cs` (zero-dep .NET 10 single-file script) replaces the broken pip-libtorrent Python path. Cross-seeding in live swarms remains a manual GUI test (Captain when convenient); checkbox refers specifically to the interop checks that can be automated, which are.
- [x] Documentation + example in `Docs/`. Shipped `Docs/bep52-example.md` (2026-04-24) covering v1/pure-v2/hybrid creation, seeding, downloading, integrity model (VerifyPieceHash branching on MetaVersion), pure-v2 wire identity via WireInfoHashHex, and cross-client interop via the new `interop_test/qbittorrent_interop.cs` automated Web UI REST driver.
